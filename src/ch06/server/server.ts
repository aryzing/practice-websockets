...
const httpsServer = createServer({
  key: readFileSync(resolve(__dirname, "localhost.key"), "utf-8"),
  cert: readFileSync(resolve(__dirname, "localhost.crt"), "utf-8"),
});

const wssServer = new Server({
  server: httpsServer,
  // Specifying a port causes ws.Server to create its own HTTP server, listening
  // at the specified port. This is a separate HTTP server to the HTTPS server
  // specified above, and is unable to process WSS requests. The HTTPS server
  // must be manually started, as ws.Server will only configure but not start
  // it.
  //
  // See relevant docs:
...
// When passing a server object to `ws.Server`, it must be started manually.
// https://github.com/websockets/ws/blob/master/doc/ws.md#new-websocketserveroptions-callback
